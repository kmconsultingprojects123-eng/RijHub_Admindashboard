...
// Frontend for Direct Hire Monitoring - displays booking quotes (type=booking) for price negotiations
let api = null;
...
(async function () {
    ...
    try {
        ...
        // Load quotes function - always filters by type=booking for Direct Hire Monitoring
        async function loadQuotes(page = 1, limit = PAGE_LIMIT, typeFilter = 'all', statusFilter = 'all', query = '', jobId = null, startDateFilter = '', endDateFilter = '') {
            ...
            try {
                showSkeleton(limit);
                const params = { page, limit, type: 'booking' }; // Always filter by booking type
                // typeFilter parameter ignored - always booking for this page
                if (statusFilter && statusFilter !== 'all') params.status = statusFilter;
                if (query) params.search = query;
                if (jobId) params.bookingId = jobId; // For Direct Hire, filter by bookingId instead of jobId
                if (startDateFilter) params.startDate = startDateFilter;
                if (endDateFilter) params.endDate = endDateFilter;
                
                const res = await api.get('/admin/quotes', { params });
                console.log('Direct Hire Quotes response:', res);
        ...
        function renderEmpty() {
            ...
            emptyDiv.innerHTML = '<div class="nk-tb-col" style="text-align:center;padding:40px;width:100%;grid-column:1/-1;"><em class="icon ni ni-inbox" style="font-size:48px;opacity:0.3"></em><p class="mt-3">No direct hire quotes found.</p></div>';
        ...
        async function showQuoteSummary() {
            ...
            try {
                // Fetch all booking quotes (Direct Hire) without pagination to get accurate stats
                const res = await api.get('/admin/quotes', { 
                    params: { page: 1, limit: 10000, type: 'booking' } 
                });
                const data = res.data;
                ...
                // Calculate statistics for Direct Hire quotes
                const stats = {
                    total: totalQuotes,
                    totalValue: 0,
                    proposed: 0,
                        ...
                        <div class="col-md-4">
                            <div class="card card-bordered h-100" style="border-left: 4px solid #6576ff;">
                                <div class="card-body">
                                    <div class="d-flex justify-content-between align-items-start">
                                        <div>
                                            <h6 class="text-muted mb-2">Total Direct Hire Quotes</h6>
